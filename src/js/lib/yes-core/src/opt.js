...
import { error, warn } from './actions';
// (function () {
...
YIUI.UICheckOpt = function (form) {
    ...
    var Return = {
        showError: function (errorMsg, type) {
            ...
            //     getDispatcher().dispatch(warn(errorMsg));
            // } else {
            throw new Error(errorMsg);
            // getDispatcher().dispatch(error(errorMsg));
            // }
        },

        _doOpt: function () {
            var self = this;
            console.log('uicheck doopt');
            ...
            for (var i in form.getComponentList()) {
                ...
                if (comp.type == YIUI.CONTROLTYPE.GRID) {
                    ...
                    } else if (comp.isRequired() && !comp.getValue()) {
...
YIUI.SaveOpt = function (form) {
    var Return = {
        doOpt: async function () {
            ...
            if (saveScript) {
                await form.eval(saveScript, { form }, null);
            }
...
YIUI.LoadOpt = function (form) {
    var Return = {
        doOpt: async function () {
            ...
            if (loadScript) {
                await form.eval(loadScript, { form: form }, null);
            }
        }
    };
    return Return;
