...
YIUI.CONTROLTYPE = {
    ...
    TEXTAREA: 246,
    UTCDATEPICKER: 254,
    SEGMENTEDCONTROL: 260,
    TABGROUP: 262,
    ICON: 270,
    SCOREBAR: 272,
    SWITCH: 275,
    SLIDER: 277,
    NUMBERINFOEDITOR: 278,
    PROGRESSINDICATOR: 279,
    COUNTDOWNVIEW: 281,
    GIFIMAGE: 282,
    VALIDATEBOX: 283,
    DYNAMIC: 20001,
...
YIUI.Chart_Type = {
    VBar: 0,/** 垂直条状图 */
    STR_VBar: "VBar",/** 垂直条状图字符串表示 */
    StackedVBar: 1,  /** 累积垂直条状图 */
    STR_StackedVBar: "StackedVBar",/** 累积垂直条状图字符串表示 */
    HBar: 2, /** 水平条状图 */
    STR_HBar: "HBar",/** 水平条状图字符串表示 */
    StackedHBar: 3, /** 累积水平条状图 */
    STR_StackedHBar: "StackedHBar", /** 累积水平条状图字符串表示 */
    Area: 4, /** 面积图 */
    STR_Area: "Area", /** 面积图字符串表示 */
    Line: 5, /** 线图 */
    STR_Line: "Line",  /** 线图字符串表示 */
    Scatter: 6, /** 散点图 */
    STR_Scatter: "Scatter",/** 散点图字符串表示 */
    Pie: 7, /** 饼图 */
    STR_Pie: "Pie", /** 饼图字符串表示 */
    Radar: 8, /** 雷达图 */
    STR_Radar: "Radar",/** 雷达图字符串表示 */
    parse: function (s) {
        var type = -1;
        if (YIUI.Chart_Type.STR_VBar.equalsIgnoreCase(s)) {
            type = YIUI.Chart_Type.VBar;
        } else if (YIUI.Chart_Type.STR_StackedVBar.equalsIgnoreCase(s)) {
            type = YIUI.Chart_Type.StackedVBar;
        } else if (YIUI.Chart_Type.STR_HBar.equalsIgnoreCase(s)) {
            type = YIUI.Chart_Type.HBar;
        } else if (YIUI.Chart_Type.STR_StackedHBar.equalsIgnoreCase(s)) {
            type = YIUI.Chart_Type.StackedHBar;
        } else if (YIUI.Chart_Type.STR_Area.equalsIgnoreCase(s)) {
            type = YIUI.Chart_Type.Area;
        } else if (YIUI.Chart_Type.STR_Line.equalsIgnoreCase(s)) {
            type = YIUI.Chart_Type.Line;
        } else if (YIUI.Chart_Type.STR_Scatter.equalsIgnoreCase(s)) {
            type = YIUI.Chart_Type.Scatter;
        } else if (YIUI.Chart_Type.STR_Pie.equalsIgnoreCase(s)) {
            type = YIUI.Chart_Type.Pie;
        } else if (YIUI.Chart_Type.STR_Radar.equalsIgnoreCase(s)) {
            type = YIUI.Chart_Type.Radar;
        }
        return type;
    },
    toString: function (type) {
        var s = null;
        switch (type) {
            case YIUI.Chart_Type.VBar:
                s = YIUI.Chart_Type.STR_VBar;
                break;
            case YIUI.Chart_Type.StackedVBar:
                s = YIUI.Chart_Type.STR_StackedVBar;
                break;
            case YIUI.Chart_Type.HBar:
                s = YIUI.Chart_Type.STR_HBar;
                break;
            case YIUI.Chart_Type.StackedHBar:
                s = YIUI.Chart_Type.STR_StackedHBar;
                break;
            case YIUI.Chart_Type.Area:
                s = YIUI.Chart_Type.STR_Area;
                break;
            case YIUI.Chart_Type.Line:
                s = YIUI.Chart_Type.STR_Line;
                break;
            case YIUI.Chart_Type.Scatter:
                s = YIUI.Chart_Type.STR_Scatter;
                break;
            case YIUI.Chart_Type.Pie:
                s = YIUI.Chart_Type.STR_Pie;
                break;
            case YIUI.Chart_Type.Radar:
                s = YIUI.Chart_Type.STR_Radar;
                break;
        }
        return s;
    }
...
YIUI.SystemField = new function () {
    ...
    Object.assign(this, systemFields);
    this.values = Object.values(systemFields);
    this.isSystemField = function (key) {
...
YIUI.Attachment_Data = new function () {
    const fields = {
        PATH: "Path",
        NAME: "Name",
        OID: "OID",
    ...
    this.values = Object.values(fields);
    this.isAttachmentField = function (key) {
        return this.values.includes(key);
    }
}();
...
YIUI.NumericUtil = (function () {
    ...
    var Return = {
        ...
        clearSubDetailData: function (form, parentGrid) {
            ...
            var compList = subDetailInfo, subComp;
            for (var i = 0, len = compList.length; i < len; i++) {
                subComp = form.getComponent(compList[i]);
                if (subComp.tagName === 'grid') {
