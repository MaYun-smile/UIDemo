...
export default YIUI.BPMStatusProxy = YIUI.extend({
    info: null,
    init: function (info) {
        this.info = info;
    },
    ...
    hasVisibleRights: function (key) {
        ...
        if( !unVisibleList || unVisibleList.length == 0 ){
            return true;
        }
        return $.inArray(key, unVisibleList) == -1;
    },
    hasOptRights: function (key) {
        var optList = this.info["optList"];
        if( !optList || optList.length == 0 ) {
            return false;
        }
        return $.inArray(key, optList) != -1;
    }
});
