...
export default YIUI.GridHandler = (function () {
    var Return = {
        /**
        ...
        doExchangeRow: function (control, rowIndex, excRowIndex) {
            ...
            var el = control.el;
            el[0].deleteGridRow(rowIndex);
            el[0].insertGridRow(rowIndex, excRow);
            el[0].deleteGridRow(excRowIndex);
            el[0].insertGridRow(excRowIndex, row);
            el.setCellFocus(excRowIndex, control.getFocusColIndex());
            this.exchangeRowSequence(form, control, rowIndex, excRowIndex);
        },
        /**
         * 单元格值改变事件
         * formID: 控件所在表单的ID
        ...
        selectRow: function (form, grid, rowIndex, colIndex, newValue) {
            ...
            if (grid.hasColExpand) {
                for (var i = 0, len = rd.bookmark.length; i < len; i++) {
                    ...
                    dataTable.setByKey(YIUI.SystemField.SELECT_FIELD_KEY, YIUI.Handler.convertValue(newValue, dataType));
                }
            } else {
        ...
        showDetailRowData: function (form, grid, rowIndex, render = true) {
                ...
                if (metaCell.hasDB) {
                    if (metaCell.isColExpand) {
                        ...
                        if (detailRowBkmk != null) {
                            ...
                            grid.setValueAt(rowIndex, i, value, false, false, render);
                        }
                    } else {
                    ...
                    // grid.setValueAt(rowIndex, i, true, false, false, render);
                    grid.setValueAt(rowIndex, i, dataTable.getByKey(YIUI.SystemField.SELECT_FIELD_KEY)? true : false, false, false, render);
                    //     }
                    // }else{
        ...
        exchangeRowSequence: function (form, grid, rowIndex, anotherIndex) {
