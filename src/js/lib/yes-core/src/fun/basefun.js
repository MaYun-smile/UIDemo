...
//     };
    ...
    funs.KickOffOperator = function (name, cxt, args) {
        ...
        var doc = Svr.Request.getSyncData(Svr.SvrMgr.ServletURL, params);
    };
    ...
    funs.GetSvrUsers = function (name,cxt,args) {
        ...
        var tableKey = "PC_LoginUsers";
        if( mode == 2 ) {
            tableKey = "MOB_LoginUsers";
        }

        var table = doc.getByKey(tableKey);

        var totalCount = YIUI.TotalRowCountUtil.getRowCount(doc,tableKey);
        form.setPara(tableKey,totalCount);
        return table;
    };
    ...
    funs.SetColumnVisible = function (name, cxt, args) {
        ...
        switch (comp.type) {
        case YIUI.CONTROLTYPE.GRID:
            var colInfoes = this.getColInfoByKey(columnKey);
            if( colInfoes.length > 0 ) {
                comp.setColumnVisible(colInfoes[0].colIndex, visible);
            }
            break;
        case YIUI.CONTROLTYPE.LISTVIEW:
                ...
                if (existsAttachment) {
                    ...
                    if (info.AttachmentOperateType == YIUI.AttachmentOperateType.NEW && OID < 0) {
                        ...
                        builder.newEmpty().then(function(emptyForm){
                            ...
                            emptyForm.setOptQueue(new YIUI.OptQueue(new YIUI.NewdOpt(emptyForm)));
                            return builder.builder(emptyForm);
                        }).then(function(data){
...
//            Svr.SvrMgr.dealWithPureForm(paras, success);
    ...
    { //BPMExtendFunction BPM扩展公式
        funs.TransferTask = function (name, cxt, args) {
            ...
            YIUI.BPMService.transferTask(WID, operatorID, createRecord);
        };
